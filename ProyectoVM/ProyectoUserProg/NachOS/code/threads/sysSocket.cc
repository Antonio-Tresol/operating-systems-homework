...
#include <iostream>
#include <sstream>
int sysSocket::fdIsValid(int fd) {
  // checks if the file descriptor is valid
  return fcntl(fd, F_GETFD) != -1 || errno != EBADF;
}
bool sysSocket::isSSL() noexcept(true) {
  if (this->SSLStruct == nullptr) {
    return false;
  }
  return true;
}
sysSocket::sysSocket(char socketType, bool isIpv6, bool isSsl) {
...
sysSocket::sysSocket(char socketType, int portToBind,
                     bool isIpv6) noexcept(false) {
  // check if socket type is valid.
  if (socketType != 's' && socketType != 'd') {
    throw SocketException("Invalid socket type", "Socket::Socket", EINVAL);
  }
...
sysSocket::sysSocket(char socketType, int portToBind, const char *certFileName,
...
void sysSocket::connectIPv4(const char *host, int portToConnect) {
  int status = -1;
  ...
  hostIpv4.sin_port = htons(portToConnect);  // host to network short (htons)
...
void sysSocket::connectIPv6(const char *host, int portToConnect) {
  int status = -1;
  ...
  // sin_port is the port number we want to connect to. it is a 16-bit integer
  hostIpv6.sin6_port = htons(portToConnect);
...
void sysSocket::Connect(const char *host, int portToConnect) {
  try {
    if (this->ipv6) {
      this->connectIPv6(host, portToConnect);
    } else {
      this->connectIPv4(host, portToConnect);
    }
  } catch (SocketException &e) {
    throw;
  }
...
void sysSocket::Connect(const char *host, const char *service) {
  int status = -1;
...
void sysSocket::Listen(int backlog) {
  int status = -1;
...
void sysSocket::bindIPv4(int portToConnect) {
  int status = -1;
  ...
  hostIpv4.sin_port = htons(portToConnect);  //  bind to port
...
void sysSocket::bindIPv6(int portToConnect) {
  int status = -1;
  ...
  hostIpv6.sin6_port = htons(portToConnect);  // port number to bind to
  struct sockaddr *hostIpv6Ptr = reinterpret_cast<sockaddr *>(&hostIpv6);
  socklen_t hostIpv6Len = sizeof(hostIpv6);
  // bind the socket to the address and port number
...
void sysSocket::Bind(int portToConnect) {
  try {
    if (this->ipv6) {
      this->bindIPv6(portToConnect);
    } else {
      this->bindIPv4(portToConnect);
    }
  } catch (SocketException &e) {
    throw;
  }
...
sysSocket *sysSocket::Accept() {
  ...
  sysSocket *newSocket = new sysSocket(newSocketFd);
  return newSocket;
}

void sysSocket::Shutdown(int mode) {
  int status = -1;
...
void sysSocket::SetIDSocket(int newId) noexcept(true) {
  this->idSocket = newId;
}

int sysSocket::sendTo(const void *message, int length, const void *destAddr) {
  int nBytesSent = -1;
...
int sysSocket::recvFrom(void *buffer, int length, void *srcAddr) {
...
void sysSocket::SSLInitServerContext() {
  SSLStartLibrary();
...
void sysSocket::SSLInitServer(const char *certFileName,
...
void sysSocket::SSLLoadCertificates(const char *certFileName,
                                    const char *keyFileName) {
...
void sysSocket::SSLShowCerts() noexcept(true) {
...
void sysSocket::SSLCreate(sysSocket *parent) {
...
void sysSocket::SSLConnect(const char *host, int portToConnect) {
  int status = -1;
  try {
...
void sysSocket::SSLConnect(const char *host, const char *service) {
  int status = -1;
  try {
...
int sysSocket::SSLRead(void *buffer, int bufferSize) {
  int nBytesRead = -1;
  try {
...
int sysSocket::SSLWrite(const void *buffer, int bufferSize) {
  int nBytesWritten = -1;
...
bool sysSocket::isReadyToRead(int timeoutSec, int timeoutMicroSec) {
...
int sysSocket::readyToReadWrite(int error) noexcept(true) {
...
void sysSocket::SSLStartLibrary() {
  SSL_library_init();
  SSL_load_error_strings();
  OpenSSL_add_all_algorithms();
}

const char *sysSocket::SSLGetCipher() {
