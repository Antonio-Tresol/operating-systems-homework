...
class sysSocket {
 public:
   ...
   */
  sysSocket(char SocketType, bool isIpv6 = false,
   ...
   * @throws SocketException if the socket can't be created.
   */
  sysSocket(char socketType, int portToBind,
            bool isIpv6 = false) noexcept(false);
  /**
   ...
   * @throws SocketException if the socket SSL structure can't be created.
   */
  sysSocket(char socketType, int portToBind, const char* certFileName,
   ...
   */
  explicit sysSocket(int socketDescriptor) noexcept(false) {
   ...
   */
  void Connect(const char* host, int portToConnect) noexcept(false);
  /**
   ...
   * @param int howManyBytes number of bytes to read
   ...
   *
   * This method attempts to write the entire string to the socket.
   * If an error occurs during the operation, a SocketException is thrown.
   *
   ...
   */
  sysSocket* Accept() noexcept(false);
  /**
   ...
   */
  void SSLConnect(const char* host, int portToConnect) noexcept(false);
  /**
   * @brief SSLConnect method uses SSL_connect sys call to connect to a server
   * @param const char* host host name
   ...
   */
  void SSLCreate(sysSocket* parent) noexcept(false);
  /**
 ...
 private:
   ...
   */
  void connectIPv4(const char* host, int portToConnect) noexcept(false);
  /**
   * @private
   ...
   */
  void connectIPv6(const char* host, int portToConnect) noexcept(false);
  /**
   * @private
   ...
   */
  void bindIPv4(int portToConnect) noexcept(false);
  /**
   * @private
   ...
   * @throws SocketException If there is an error binding to the socket.
   */
  void bindIPv6(int portToConnect) noexcept(false);
  /**
   * @private
