...
void Machine::OneInstruction(Instruction *instr) {
  ...
  switch (instr->opCode) {
    ...
    case OP_SB:
      if (!machine->WriteMem(
              (unsigned)(registers[(int)instr->rs] + instr->extra), 1,
              registers[(int)instr->rt]))
        return;
      break;
    ...
    case OP_SH:
      if (!machine->WriteMem(
              (unsigned)(registers[(int)instr->rs] + instr->extra), 2,
              registers[(int)instr->rt]))
        return;
      break;
    ...
    case OP_SW:
      if (!machine->WriteMem(
              (unsigned)(registers[(int)instr->rs] + instr->extra), 4,
              registers[(int)instr->rt]))
        return;
      break;
    ...
    case OP_SWL:
      ...
      switch (tmp & 0x3) {
        ...
        case 3:
          value =
              (value & 0xffffff00) | ((registers[(int)instr->rt] >> 24) & 0xff);
          break;
      }
