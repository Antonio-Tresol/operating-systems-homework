...
MailBox::~MailBox() {
  ...
  // IPC_RMID flag indicates that the message queue should be destroyed
  // The third argument is a pointer to a msqid_ds structure, which is used to return information about the message queue.
...
int MailBox::send( long type, void * buffer, int numBytes ) {
  ...
  m.type = type;
  memcpy( static_cast<void*>(m.data), buffer, numBytes ); // Copy the message data to the message buffer
